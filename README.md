...
You can override built-in operators like `=~` for custom logic, such as implementing regex matching. For example, to enable `=~` with Go's RE2 regex:

```go
func(info expr.OpInfo, ctx *expr.Context) (expr.Value, error) {
    switch info.Op {
    case expr.OpRegex:  // Handles the =~ operator
        str := info.Left.String()
        pat := info.Right.String()
        matched, err := regexp.MatchString(pat, str)
        if err != nil {
            return expr.Bool(false), nil  // Treat invalid regex as no match
        }
        return expr.Bool(matched), nil
    // ... other cases
    }
    return expr.Undefined, nil
}
